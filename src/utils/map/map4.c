...
/*   Updated: 2025/06/15 15:22:25 by pvcordeiro       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if (x < 0 || y < 0 || x > m_size.width || y > m_size.height || !map[y] || !map[y][x] ||
        map[y][x] == 'F' || map[y][x] == '1' || map[y][x] == ' ')
        return ;
    map[y][x] = 'F';
...
static bool check_map_boundary(char **map_copy, int x, int y, t_map *map)
{
	...
	i = -1;
    while (++i < 4)
        if (x + dx[i] < 0 || y + dy[i] < 0 || y + dy[i] >= map->size.height || !map_copy[y + dy[i]][x + dx[i]] || 
            map_copy[y + dy[i]][x + dx[i]] == ' ')
            return (false);
    return (true);
}
...
void ft_backtrack(t_game *game)
{
    ...
    while (++y < map->size.height)
    {
		x = -1;
        while (map_copy[y][++x])
            if (map_copy[y][x] == 'F' && !check_map_boundary(map_copy, x, y, map))
