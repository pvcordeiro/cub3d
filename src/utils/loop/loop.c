...
/*   Updated: 2025/05/18 17:50:24 by paude-so         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	loop(void)
{
	...
	if (cub3d()->game.hud.enabled)
		ftm_draw_rectangle(cub3d()->window.canvas, (t_coords){20, 20, 0}, (t_size){600, 100}, (t_ftm_rectangle){0x55000000, 0x55EFBF04, (t_size){2, 2}});
	ftm_update_window(&cub3d()->window);
	update_fps(&cub3d()->game);
	render_overlay(&cub3d()->game);
}
