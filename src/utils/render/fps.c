...
/*   Updated: 2025/06/14 11:37:41 by pvcordeiro       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_fps_data(t_fps_data *fps_data, double delta_time)
{
	...
	if (current_time > fps_data->last_update_time)
	{
		...
		snprintf(fps_data->fps_string, 50, "FPS: %.0f | Frame: %.2f ms",
			fps_data->fps, fps_data->frametime);
	}
}
...
void	render_fps_counter(t_fps_data *fps_data, t_ftm_window *window)
{
	if (!fps_data || !window || !window->canvas)
		return ;
	ftm_draw_rectangle(window->canvas, (t_coords){10, 10, 0, 0}, (t_size){180,
		20}, (t_ftm_rectangle){0xAA000000, 0x00000000, (t_size){0, 0}});
	ftm_update_window(window);
	mlx_string_put(window->display, window->win, 20, 25, 0xFFFFFFFF,
		fps_data->fps_string);
}
