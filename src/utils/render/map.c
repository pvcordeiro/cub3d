...
static void	render_rays(t_ftm_image *canvas, t_game *game, t_player *player,
		t_coords coords, t_size entity_size)
{
	...
	while (++i < PLAYER_RAYS)
	{
		...
		ftm_draw_line_angle(canvas, coords, player->rays[i].length * scale,
			game->minimap.player_ray_color);
	}
}
...
static void	render_entity(t_ftm_image *canvas, t_game *game, t_entity *entity,
		t_coords coords, t_size entity_size)
{
	unsigned	color;
	t_coords	new_coords;
	...
	ftm_draw_rectangle(canvas, new_coords, entity_size, (t_ftm_rectangle){color,
		color, (t_size){1, 1}});
}

static void	render_entities(t_ftm_image *canvas, t_game *game, t_coords coords,
		t_size size)
{
	t_list		*entity_item;
	t_entity	*entity;
	...
	add_to_last.width = (size.width - (entity_size.width
				* game->map->size.width)) / 2;
	add_to_last.height = (size.height - (entity_size.height
				* game->map->size.height)) / 2;
	entity_item = game->entities;
	while (entity_item)
	{
		entity = entity_item->data;
		if (entity != (t_entity *)game->player)
			render_entity(canvas, game, entity, (t_coords){coords.x
				+ add_to_last.width, coords.y + add_to_last.height, 0, 0},
				entity_size);
		entity_item = entity_item->next;
	}
	render_entity(canvas, game, (t_entity *)game->player, (t_coords){coords.x
		+ add_to_last.width, coords.y + add_to_last.height, 0, 0}, entity_size);
}

void	render_minimap(t_game *game, t_ftm_image *canvas, t_coords coords,
		t_size size)
{
	ftm_draw_rectangle(canvas, coords, size,
		(t_ftm_rectangle){game->minimap.background_color,
		game->minimap.border_color, (t_size){1, 1}});
