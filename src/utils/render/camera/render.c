...
/*   Updated: 2025/05/20 19:50:09 by afpachec         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static unsigned	pixel_modifier(void *data, unsigned pixel)
{
	...
	pmd = data;
	gradient = fmin(pmd->ray_size->height / (pmd->canvas->size.width / 1.5), 1.0);
...
static t_ftm_pitc_config	get_pitc_config(t_get_pitc_config_config gpcc)
{
	...
	return ((t_ftm_pitc_config){
			...
			(t_coords){(int)(x_of_hit * gpcc.image->size.width) + 1, gpcc.image->size.height, 0},
			true,
			*gpcc.ray_size,
...
void	draw_ray_line(t_ftm_image *canvas, t_camera *camera, t_ray ray, int i)
{
	...
	if (!hit_entity_image)
		return ;
	ray_size.height = canvas->size.height / (fmax(ray.distance, 0.1) * cos(ft_normalize_angle((camera->entity->coords.yaw - (camera->fov / 2)) - camera->entity->coords.yaw) * (PI / 180.0)));
