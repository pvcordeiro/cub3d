...
/*   Updated: 2025/06/21 23:49:09 by afpachec         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	int player_index)
{
	...
	if (i == 2)
		return ((t_player_canvas_data){(t_size){cs.width / 2, cs.height / 2}, (t_coords){player_index * (cs.width / 2), (cs.height / 2) / 2, 0}});
	if (i == 3)
	{
		if (player_index == 0 || player_index == 1)
			return ((t_player_canvas_data){(t_size){cs.width / 2, cs.height / 2}, (t_coords){player_index * cs.width / 2, 0, 0}});
		return ((t_player_canvas_data){(t_size){cs.width / 2, cs.height / 2}, (t_coords){(cs.width / 2) / 2, cs.height / 2, 0}});
	}
	if (player_index == 0 || player_index == 1)
		return ((t_player_canvas_data){(t_size){cs.width / 2, cs.height / 2}, (t_coords){player_index * cs.width / 2, 0, 0}});
...
void	render_players_game(t_game *game, t_ftm_window *window)
{
	...
	while (++i < PLAYER_MAX)
	{
		...
		if (!game->players[i]->canvas
			...
			|| game->players[i]->last_canvas_pos.x != player_canvas_data.coords.x
			|| game->players[i]->last_canvas_pos.y != player_canvas_data.coords.y)
		{ 
			ftm_put_image_to_window_pitc(window,
				get_sprite_image(game->background_sprite),
				(t_ftm_pitc_config){
					.coords = {0}, .crop = false, .resize = true,
					.size = window->size});
