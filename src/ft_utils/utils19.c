...
/*   Updated: 2025/06/15 01:33:22 by afpachec         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_ray(t_dda_raycast_data *ray, t_dda_raycast_config ddarc)
{
	...
	ray->deltadist.x = ft_ternary_double(fabs(ray->rdir.x) < EPSILON,
			MAX_RAY_LENGTH, fabs(1.0 / ray->rdir.x));
	ray->deltadist.y = ft_ternary_double(fabs(ray->rdir.y) < EPSILON,
			MAX_RAY_LENGTH, fabs(1.0 / ray->rdir.y));
...
static void	set_aftermath(t_dda_raycast_data *r)
{
	if (!r->side)
	{
		r->length = ft_ternary_double(fabs(r->rdir.x) < EPSILON, MAX_RAY_LENGTH,
	...
	r->length = ft_ternary_double(fabs(r->rdir.y) < EPSILON, MAX_RAY_LENGTH,
...
t_raycast	ft_dda_raycast(t_dda_raycast_config ddarc)
{
	...
	if (!ray.hit)
		return ((t_raycast){MAX_RAY_LENGTH, ddarc.start_pos.yaw, NULL, 0, 0,
			(t_coords){0, 0, 0}});
	set_aftermath(&ray);
	return ((t_raycast){
		ray.length,
